                ...
                try {
                    $stmt = $pdo->prepare(
                        "INSERT INTO subscriptions
                         (name, days, sessions, invites, freeze_days, price, discount_percent, price_after_discount, discount_end_date)
                         VALUES (:n, :d, :s, :i, :fz, :p, :dp, :pad, :ded)"
                    );
                    $stmt->execute([
                        ':n'   => $name,
                        ':d'   => $days,
...
<head>
    <meta charset="UTF-8">
    <title>إدارة الاشتراكات - <?php echo htmlspecialchars($siteName); ?></title>
    <style>
        * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
        ...
        .form-row-line{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:12px;align-items:flex-end;margin-bottom:14px;}
        ...
        .table-wrapper{margin-top:18px;border-radius:20px;border:1px solid var(--border);overflow:hidden;}
...
<div class="page">
    <div class="header-bar">
        <div>
            ...
            <div class="title-sub">
                تسجيل أنواع الاشتراكات (المدة، عدد مرات التمرين، عدد الدعوات، أيام الـ Freeze، الأسعار والخصومات) مع عرض عدد المشتركين في كل اشتراك.
            </div>
        </div>
        <div>
        ...
        <div class="table-wrapper">
            <table>
                ...
                <?php if (!$subscriptions): ?>
                    <tr>
                        <td colspan="11" class="empty">لا توجد اشتراكات مسجلة حتى الآن.</td>
                    </tr>
                <?php else: ?>
...
<script>
    ...
    // تعبئة نموذج التعديل
    function fillEditForm(id, name, days, sessions, invites, freezeDays, price, discount, discountEndDate) {
