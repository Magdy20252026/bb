...
function getStatsForRange(PDO $pdo, string $start, string $end): array {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT
                COUNT(*)                     AS c,
                COALESCE(SUM(new_subscription_amount), 0) AS s
            FROM renewals_log
        ...
        $stmt->execute([':start' => $start, ':end' => $end]);
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $result['total_expenses'] = (float)($row['s'] ?? 0.0);
        }

        // حصص التمرينة الواحدة (باستخدام عمود single_paid بعد الإصلاح)
        $stmt = $pdo->prepare("
            SELECT
        ...
        <?php else: ?>
            <div class="grid">
                ...
                <div class="stat-block">
                    ...
                    <div class="stat-row">
                        <span class="label">إجمالي قيمة الاشتراكات المجددة:</span>
